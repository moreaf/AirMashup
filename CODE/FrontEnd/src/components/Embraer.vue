    ...
    <v-flex lg3 v-for="embraerModel in embraerModels">
       <v-card  v-bind:key="embraerModel.id" class="xxx indigo elevation-20">
          ...
          <v-card-actions class="align-center justify-space-around">
            <v-btn class ="elevation-20 mb-3" color="yellow darken-4" dark @click="loadTour(embraerModel.name,embraerModel.lat,embraerModel.lon,embraerModel.range,embraerModel.description)" >LAUNCH MODEL</v-btn>
            <v-btn class ="elevation-20 mb-3" color="light-blue lighten-1" dark @click="initOrbit(embraerModel.name)">ORBIT</v-btn>
